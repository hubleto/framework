 ...
 */
class Token extends \Hubleto\Legacy\Core\Model {
  ...
  public function registerTokenType($type) {
    if (!in_array($type, $this->tokenTypes)) {
      ...
      throw new \Hubleto\Legacy\Core\Exceptions\GeneralException("Duplicate token type: {$type}");
    }
  }
  ...
  public function generateToken($tokenSalt, $tokenType, $validTo = NULL) {
    ...
    if (!in_array($tokenType, $this->tokenTypes)) {
      throw new \Hubleto\Legacy\Core\Exceptions\GeneralException("Unknown token type: {$tokenType}");
    }
    ...
    if (strtotime($validTo) < time()) {
      throw new \Hubleto\Legacy\Core\Exceptions\GeneralException("Token validity can not be in the past.");
    }
  ...
  public function validateToken($token) {
    ...
    if (!is_array($tokenData)) {
      throw new \Hubleto\Legacy\Core\Exceptions\InvalidToken($token);
    }
