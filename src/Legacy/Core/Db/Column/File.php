...
class File extends \Hubleto\Legacy\Core\Db\Column
{
  ...
  public function normalize(mixed $value): mixed
  {
    ...
    if (empty($renamePattern)) {
      ...
      $fileName = \Hubleto\Legacy\Core\Helper::str2url($tmpParts['filename']) . '.' . $tmpParts['extension'];
    } else {
      $tmpParts = pathinfo($fileName);
      ...
      $fileName = str_replace("{%BASENAME_ASCII%}", \Hubleto\Legacy\Core\Helper::str2url($tmpParts['basename']), $fileName);
      $fileName = str_replace("{%FILENAME%}", $tmpParts['filename'], $fileName);
      $fileName = str_replace("{%FILENAME_ASCII%}", \Hubleto\Legacy\Core\Helper::str2url($tmpParts['filename']), $fileName);
