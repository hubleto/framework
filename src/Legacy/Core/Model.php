...
use Hubleto\Legacy\Core\Db\DataType;
use Hubleto\Legacy\Core\Db\Query;
use Hubleto\Legacy\Core\Exceptions\DBException;
use Hubleto\Legacy\Core\Exceptions\RecordSaveException;
use Hubleto\Legacy\Core\ViewsWithController\Form;
use Hubleto\Legacy\Core\ViewsWithController\Table;
use Closure;
use Exception;
use ReflectionClass;
...
class Model
{
  ...
  /** @property array<string, \Hubleto\Legacy\Core\Db\Column> */
  ...
  public function dropTableIfExists(): \Hubleto\Legacy\Core\Model
  {
  ...
  /** @return array<string, \Hubleto\Legacy\Core\Db\Column> */
  public function describeColumns(): array
  {
    $columns = [];

    if (!$this->isJunctionTable) {
      $columns['id'] = new \Hubleto\Legacy\Core\Db\Column\PrimaryKey($this, 'ID', 8);
    }
  ...
  public function describeTable(): \Hubleto\Legacy\Core\Description\Table
  {
    ...
    $description = new \Hubleto\Legacy\Core\Description\Table();
    foreach ($columns as $columnName => $column) {
      if (!$column->getHidden()) {
  ...
  {
    $description = new \Hubleto\Legacy\Core\Description\Form();
