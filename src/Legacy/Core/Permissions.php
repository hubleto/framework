...
class Permissions {
   ...
   */
  protected \Hubleto\Legacy\Core\Loader $app;
  ...
  {
    return new \Hubleto\Legacy\Models\UserRole($this->app);
  }
  ...
  */
  ...
  public function check(string $permission) {
    if (!$this->granted($permission) && !$this->granted(str_replace('\\', '/', $permission))) {
      throw new \Hubleto\Legacy\Core\Exceptions\NotEnoughPermissionsException("Not enough permissions ({$permission}).");
    }
  }
